    ...
    <div className="bg-gradient-to-r from-blue-50 via-white to-blue-50 min-h-screen py-16 px-6 md:px-20">
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-200 hover:shadow-3xl transition-shadow duration-500">
        ...
        <div className="relative group overflow-hidden rounded-t-3xl">
          <img
            src={packageData.thumbnail || "https://i.ibb.co/R4Z4VYXD/sajekbanner.jpg"}
        ...
        <div className="flex gap-6 p-6 px-10 bg-blue-50">
          {[packageData.image1, packageData.image2].map(
            (img, idx) =>
              img && (
                <div key={idx} className="flex-1 overflow-hidden rounded-xl shadow-md group cursor-pointer">
                  <img
                    src={img}
        ...
        <div className="p-10 space-y-6">
          <h1 className="text-4xl font-extrabold text-blue-900">{packageData.title}</h1>
          <p className="text-gray-700 text-lg font-medium">{packageData.subtitle}</p>
          ...
          <div className="flex flex-wrap gap-6 text-gray-800 font-semibold text-base">
            <div className="flex flex-col">
              ...
              <span className="text-xl text-blue-700">${packageData.price}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-sm text-gray-500 uppercase tracking-wide">Duration</span>
              <span>{packageData.duration}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-sm text-gray-500 uppercase tracking-wide">Departure</span>
              <span>{new Date(packageData.departureDate).toLocaleDateString()}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-sm text-gray-500 uppercase tracking-wide">Return</span>
              <span>{new Date(packageData.returnDate).toLocaleDateString()}</span>
            </div>
          </div>
